...
- KD14. **Места переключаются тапом по постоянно видимому ряду вкладок, разборка — тапом по предмету** (session-settled: user-approved — chosen over вертикальный свайп для смены места и пинч для разборки: оба конфликтуют с браузерными жестами мобильного веба, пинч недоступен большим пальцем одной руки, а видимый ряд вкладок сам показывает игроку структуру мест). Governs R2, R5.
...
- R2. Контейнеры сгруппированы по местам: дом, Wertstoffinsel, Wertstoffhof, спецприём. Лента показывает контейнеры одного места за раз. Места переключаются тапом по ряду вкладок, который виден постоянно и перечисляет все открытые места, а не только активное. Ни на одном месте лента не длиннее пяти контейнеров.
...
- R4. Игра растёт по четырём тирам, каждый из которых вводит новое место в порядке реальной структуры города: дом → Wertstoffinsel → Wertstoffhof → спецприём. Первый тир содержит только дом и не показывает ряд вкладок; вкладки появляются вместе со вторым местом. Новый тир вводится, когда предыдущий пройден, и ранее открытые места остаются доступны.
- R5. Предмет может нести признак «разборный», независимый от признака «пограничный»: такой нельзя отправить целиком. Тап по предмету разделяет его на части, дальше каждая часть сортируется отдельно и может уходить в контейнер другого места. Механика разборки вводится начиная со второго тира.
...
flowchart TB
  ...
  P2["Место 2 — Wertstoffinsel<br/>стекло по цветам, текстиль,<br/>лёгкие упаковки до 2027<br/>появляется ряд вкладок"]
  ...
  SPLIT["Разборные объекты<br/>тап разделяет, вводятся со второго тира,<br/>части могут уходить в разные места"]
  P2 -.-> SPLIT
...
- F1. Обычный уровень
  ...
  - **Шаги:** предметы падают по одному; игрок при необходимости тапает вкладку нужного места, затем свайпает ленту; попадание засчитывается, промах вызывает объяснение и продолжение.
...
- F2. Разборный объект
  ...
  - **Шаги:** игрок тапает по предмету, тот распадается на части; каждая часть сортируется отдельно, при необходимости со сменой места.
...
- AE4. **Covers R3, R5, R9.** Дано: падает конвертик от чайного пакетика с ламинированной стороной. Предмет несёт признак «пограничный», но не «разборный»: он падает медленнее и выделен, тап по нему ничего не разделяет, а при ошибке объяснение называет причину — композитный материал.
...
- Объём первого релиза складывается из четырёх потоков: сборка механик, включая ряд вкладок и оба финала, — порядка пяти выходных; подготовка контента — сверка каждого предмета и маршрута с AWM плюс тексты объяснений на DE и EN — оценивается отдельно и масштабируется от числа предметов на тир, которое пока не задано; обязательный правовой минимум R22; подготовка и размещение публикаций по R20 с лагом на модерацию. Сбор статистики R21 в критический путь не входит: по KD12 он переносится первым.
...
- Добавлять ли вертикальный свайп по области ленты как необязательный ускоритель смены места поверх вкладок — и переживает ли он подавление браузерных дефолтов на реальном iOS.
- Где живёт ряд вкладок относительно ленты и падающих предметов, чтобы тап по вкладке не перехватывался областью падения.
