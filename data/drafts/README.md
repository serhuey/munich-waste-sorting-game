...
Про поле «код варианта»: игроку он не показывается, движку нужен, чтобы
отличать версии предмета внутри файла. Заполняется сам из названия варианта —
трогать его нужно, только если хочется своё имя.
...
4. `explanation.de` и `explanation.en` — почему так. Одно-два предложения.
   Это **общее пояснение предмета**: что тут вообще важно и, если вариантов
   несколько, чем они отличаются. У варианта и у части есть свои поля
   пояснения — необязательные. Заполняйте их, когда «почему сюда» у вариантов
   разное: у банки с залогом и без залога причины разные, и одной фразой на
   двоих они не объясняются. Игра показывает всё, что есть, от частного к
   общему.
...
- `awm` — запись Abfalllexikon: у неё есть ключ, и сборка сверяет её со снимком,
  ловя изменения правил.
- `page` — обычная страница AWM, которой в лексиконе нет: что стоит на острове,
  что и куда переезжает в 2027. Ключа нет, вместо него название страницы.
- `law` — норма федерального закона: приём батареек и электроники в магазине
  держится на ней, а не на AWM.

У одного предмета их может быть несколько, и каждый подписывается отдельно.
